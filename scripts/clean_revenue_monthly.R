...
# Long format with 12 month receipt columns + source + revenue heading.
# =============================================================================
...
MONTH_LABEL <- c(
  "साउन"="received_saun", "भदौ"="received_bhadau", "आस्बिन"="received_asoj",
  "कार्तिक"="received_kartik", "मार्ग"="received_mangsir", "पौष"="received_poush",
  "माघ"="received_magh", "फागुन"="received_falgun", "चैत्र"="received_chaitra",
  "बैशाख"="received_baisakh", "जेठ"="received_jestha", "असार"="received_asar"
)
...
clean_one <- function(file, lookup, receipt_type) {
  ...
  src_col <- which(r5 == "स्रोत")
  rh_top <- which(r5 == "राजस्व शीर्षक")
  if (!length(lg_top) || !length(src_col) || !length(rh_top))
    stop("missing top-level headers")

  lg_code_col <- which(r6 == "संकेत" & seq_along(r6) >= lg_top[1] & seq_along(r6) < src_col[1])
  lg_name_col <- which(r6 == "नाम"   & seq_along(r6) >= lg_top[1] & seq_along(r6) < src_col[1])
  ...
  if (!length(lg_code_col) || !length(lg_name_col) ||
    ...
    stop("missing sub-headers in row 6")
  ...
  for (nm in names(MONTH_LABEL)) {
    p <- which(r6 == nm)
    if (length(p)) month_cols[[MONTH_LABEL[[nm]]]] <- p[1]
  }
  if (!length(month_cols)) stop("no month columns detected")

  monthly_vals <- as.data.frame(lapply(month_cols, function(c) parse_np_num(raw[[c]])))
  names(monthly_vals) <- names(month_cols)

  df <- bind_cols(
    tibble(
      lg_code_raw          = np_digits_to_ascii(raw[[lg_code_col]]),
      lg_name_np           = as.character(raw[[lg_name_col]]),
      source_np            = as.character(raw[[src_col[1]]]),
...
main <- function() {
  ...
  for (rt in names(RECEIPT_VARIANTS)) {
    ...
    message(sprintf("  %s -> %d rows | %d LGs | unmatched: %d",
