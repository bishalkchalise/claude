...
#
# National rollup by funding source. Source columns detected from row 5
# so unknown / extra sources don't break the run.
# =============================================================================
...
clean_one <- function(file) {
  ...
  val_cols <- list()
  for (nm in names(SOURCE_PREFIX)) {
    pos <- which(r5 == nm)
    if (length(pos)) {
      c <- pos[1]
      pref <- SOURCE_PREFIX[[nm]]
      if (c + 1 <= ncol(raw) &&
  ...
  if (!length(val_cols)) stop("no source columns detected")
